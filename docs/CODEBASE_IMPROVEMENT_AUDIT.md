...
- [~] High #5/#7: major use-case consistency cleanup completed (`LoginUseCase`, `GetDashboardDataUseCase`, `GetBudgetsUseCase`, `GetTransactionsUseCase` wired in ViewModels) and transaction-form helpers extracted (account preservation + submission/id-resolution + auto-classify apply + counterparty-search state handling + edit-load state mapping); additional decomposition still possible.
